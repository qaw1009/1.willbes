...
@section('layer_content')
    ...
    <form class="form-horizontal searching" id="search_form_modal" name="search_form_modal" method="POST" onsubmit="return false;" novalidate>
        <div class="form-group">
            <div class="row mt-5">
                <div class="col-md-2">
                    {!! html_site_select('', 'search_modal_site_code', 'search_modal_site_code', 'mr-5', '운영 사이트', 'required', '', true) !!}
                </div>
                <div class="col-md-6 form-inline">
                    <select class="form-control mr-5" id="search_modal_popup_disp" name="search_modal_popup_disp" title="노출섹션">
                        <option value="">노출섹션</option>
                        @foreach($arr_popup_disp_ccd as $key => $val)
                            <option value="{{$key}}">{{$val}}</option>
                        @endforeach
                    </select>
                    <select class="form-control mr-5" id="search_modal_is_use" name="search_modal_is_use" title="사용여부">
                        <option value="">사용여부</option>
                        <option value="Y">사용</option>
                        <option value="N">미사용</option>
                    </select>
                    <input type="text" class="form-control mr-15" id="search_modal_value" name="search_modal_value" title="검색어" style="width: 300px;" placeholder="통합 검색 가능">
                </div>
            </div>
        </div>
    </form>
                ...
                <table id="list_modal_table" class="table table-striped table-bordered">
                    ...
                    @foreach($data as $row)
                        <tr>
                            ...
                            <td>{!! str_replace(',', '<br/>', $row['CateCode']) !!}</td>
                            <td>{{ $row['DispName'] }}<span class="hide">{{ $row['DispCcd'] }}</span></td>
                            <td>{{ $row['PopUpName'] }}</td>
                            <td><img src="{{$row['PopUpFullPath']}}{{$row['PopUpImgName']}}" width='100%' height='30%' alt="배너이미지"></td>
                            <td>
                                {{$row['DispStartDatm']}} ~ {{$row['DispEndDatm']}}
                            </td>
                            <td>
                                @if($row['IsUse'] == 'Y') 사용 @elseif($row['IsUse'] == 'N') <span class="red">미사용</span> @endif
    ...
    <script type="text/javascript">
        ...
        function datatableSearching() {
            $datatable_modal
                .columns('.searching').flatten().search($search_form_modal.find('input[name="search_modal_value"]').val())
                ...
                .column('.searching_is_use').search($search_form_modal.find('select[name="search_modal_is_use"]').val())
                .draw();
        }
