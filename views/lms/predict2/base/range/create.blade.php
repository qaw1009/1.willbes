    ...
    <div class="x_panel">
        ...
        <div class="x_content">
            <form class="form-table" id="regi_form" name="regi_form" method="POST" onsubmit="return false;" novalidate>
                {!! csrf_field() !!}
                {!! method_field($method) !!}
                <input type="hidden" name="idx" value="{{ $data['PaIdx'] }}">
                ...
                <form class="form-table" id="regi_sub_form" name="regi_sub_form" method="POST" onsubmit="return false;" novalidate>
                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach($chData as $row)
                            <tr data-chapter-idx="{{ $row['PalIdx'] }}">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $regi_form.submit(function() {
                ...
                var _url = '{{ site_url('/predict2/base/range/store/') }}';
                ajaxSubmit($regi_form, _url, function(ret) {
                    if(ret.ret_cd) {
                        notifyAlert('success', '알림', ret.ret_msg);
                        location.replace('{{ site_url('/predict2/base/range/create/') }}' + ret.ret_data.dt.idx + getQueryString());
                    }
            ...
            $('.act-searchCate').on('click', function() {
                ...
                $('.act-searchCate').setLayer({
                    'url' : '{{ site_url() }}' + 'predict2/base/code/moCate/?siteCode=' + $('[name=siteCode]').val(),
                    'width' : 1100
                });
            });
