    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $datatable = $list_table.DataTable({
                ...
                buttons: [],
                ajax: {
                    'url' : '{{ site_url('/predict/passline/listAjax') }}',
                    'type' : 'POST',
                    'data' : function(data) {
                ...
                columns: [
                    ...
                    {'data' : null, 'render' : function(data, type, row, meta) {
                            return '<a class="blue" href="{{ site_url('/predict/request/create/') }}'+row.PredictIdx+'" target="_blank">[' + row.PredictIdx + '] ' + row.ProdName + '</a>';
                        }},
                    ...
                    {'data' : null, 'render' : function(data, type, row, meta) {
                            return '<button type="button" class="btn btn-primary btn-detail" data-predict-idx="'+ row.PredictIdx + '">합격선등록</button>';
                        }},
                ]
            });

            $list_form.on('click', '.btn-detail', function () {
                location.href='{{ site_url("/predict/passline/detail") }}/' + $(this).data('predict-idx') + dtParamsToQueryString($datatable)
            });
        });
    </script>
@stop