...
class NpayProduct extends \app\controllers\FrontController
{
    ...
    public function index($params=[])
    {
        try {
            ...
            $write = "id\tgoods_type\ttitle\tprice_pc\tprice_mobile\tnormal_price\tlink\tmobile_link\timage_link\tcategory_name1\tnaver_category\tcondition\tbrand\tmaker\tsearch_tag\tshipping" . PHP_EOL;

            foreach ($data as $row) {
                $write .= $row['id'] . "\t" . "P\t" . $row['title'] . "\t" . $row['price_pc'] . "\t" . $row['price_mobile'] . "\t" . $row['normal_price'] . "\t" . $row['link'] . "\t" . $row['mobile_link'] . "\t" . $row['image_link'] . "\t" . $row['category_name1']
