...
class Point extends \app\controllers\FrontController
{
    ...
    public function index($params = [])
    {
        ...
        $point_status = strtolower(element('stab', $arr_input, 'save'));    // 적립 : save, 사용 : use
        ...
        // 기본 조회 조건
        $arr_condition = ['EQ' => ['PS.PointType' => $point_type]];

        // 총 적립 포인트
        $total_save_point = array_get($this->pointFModel->listMemberSavePoint('ifnull(SUM(PS.SavePoint), 0) as SavePoint', $arr_condition), '0.SavePoint', 0);
        ...
        // 총 사용 포인트
        $total_use_point = array_get($this->pointFModel->listMemberUsePoint('ifnull(SUM(PU.UsePoint), 0) as UsePoint', $arr_condition), '0.UsePoint', 0);

        // 목록 조회
        $list_method = ucfirst($point_status);
        $date_column = ['save' => 'PS.SaveDatm', 'use' => 'PU.UseDatm'];
        $arr_orderby = ['save' => ['PS.PointIdx' => 'desc'], 'use' => ['PU.PointUseIdx' => 'desc']];
        $arr_condition = array_merge_recursive($arr_condition, [
            'EQ' => ['S.SiteGroupCode' => element('site_group', $arr_input)],
            'BDT' => [$date_column[$point_status] => [element('search_start_date', $arr_input), element('search_end_date', $arr_input)]]
        ]);
