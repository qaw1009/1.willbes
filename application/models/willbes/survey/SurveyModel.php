...
class SurveyModel extends WB_Model
{
    ...
    public function update()
    {
        try {
            ...
            $register_data = $this->predictFModel->findPredictRegister2($arr_condition, 'PrIdx, TakeNumber');
            if ($register_data['TakeNumber'] != $this->input->post('TakeNumber')) {
                $arr_condition = [
                    'EQ' => [
                        'PredictIdx' => $PredictIdx,
                        'TakeNumber' => $this->input->post('TakeNumber'),
 ...
 */
    ...
    public function getAreaForLineData($PredictIdx, $take_mock_part, $take_area)
    {
        $column = "
		    ...
		    WHERE B.PointRank BETWEEN 0 AND ROUND((pg.PickNum / pg.TakeNum),2)
