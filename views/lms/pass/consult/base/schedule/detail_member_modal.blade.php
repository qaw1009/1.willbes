...
@section('layer_content')
            ...
            <div class="x_content form-group-sm">
                <div class="form-group">
                    ...
                    <div class="form-inline col-md-4">
                        <div class="radio">
                            @if(empty($consult_status) === false)
                                @foreach($consult_status as $st_key => $st_val)
                                    <input type="radio" id="is_consult_{{$st_key}}" name="is_consult" class="flat" value="{{$st_key}}" required="required" @if($data['IsConsult'] == $st_key)checked="checked"@endif/>
                                    <label for="is_consult_{{$st_key}}" class="input-label">{{$st_val}}</label>
                                @endforeach
                            @endif
                            {{--
    ...
    <script type="text/javascript">
        ...
        $('.btn-modal-modify').on('click', function() {
            var _url = '{{ site_url($default_path . '/schedule/storeDetailMember/') }}';

            ajaxSubmit($list_modal_form, _url, function(ret) {
                if(ret.ret_cd) {
                    notifyAlert('success', '알림', ret.ret_msg);
                    location.replace('{{ site_url($default_path . '/member/') }}' + getQueryString());
                }
