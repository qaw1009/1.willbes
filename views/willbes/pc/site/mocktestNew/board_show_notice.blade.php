                                    ...
                                    @if(empty($board_data['AttachData']) === false)
                                        @foreach($board_data['AttachData'] as $row)
                                            @if($row['FileType'] == 0)
                                                ...
                                                {{--<a href="{{front_url('/mocktestNew/boardFileDownload?path=').urlencode($row['FilePath'].$row['FileName']).'&fname='.urlencode($row['RealName']).'&board_idx='.$board_idx }}" target="_blank">--}}
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#btn_list').click(function() {
                location.href = '{!! front_url('/mocktestNew/listNotice/cate/'.$__cfg['CateCode'].'?'.$get_params) !!}';
            });
        });
    </script>
@stop