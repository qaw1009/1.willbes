                    ...
                    <div class="reserveTableList">
                        <div class="reserveTable tx-gray">
                            <div class="table-row top p_re">
                                <div class="table-cell w-data w-data-span3 tx-left tx-gray pl15">조회된 {{ $consult_name }} 예약이 없습니다.</div>
                            </div>
                        </div>
                    </div>
                @else
                    @foreach($arr_base['list'] as $row)
                        <div class="reserveTableList">
                            <div class="reserveTable tx-gray">
                                <div class="table-row top p_re">
                                    <div class="table-cell w-tit bg-light-white strong"><p>{{ $consult_name }} 예약 일시</p></div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('.btn-my-cancel').click(function () {
            if (!confirm('{{ $consult_name }} 예약을 취소하시겠습니까?')) { return false; }
            var _url = '{{ front_url($default_path . '/cancel') }}';
            var _data = {
