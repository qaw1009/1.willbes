...
@section('layer_header')
    <form class="form-table form-horizontal" id="regi_form" name="regi_form" method="POST" onsubmit="return false;" novalidate>
        {!! csrf_field() !!}
        {!! method_field($method) !!}
        <input type="hidden" name="idx" value="{{ $mock_data['PcIdx'] }}">
        ...
        @section('layer_content')
            ...
            <table class="table table-bordered modal-table">
                <tr>
                    ...
                    <td style="width:35%">
                        {!! html_site_select((empty($mock_data['SiteCode']) === true ? '' : $mock_data['SiteCode']), 'subject_site', 'site', 'show', '운영 사이트', '', '', false) !!}
                    </td>
                ...
                <tr>
                    <th>최종수정자</th>
                    <td>{{ (empty($arr_adminInfo['UpdAdminName']) === true) ? '' : $arr_adminInfo['UpdAdminName'] }}</td>
                    <th>최종수정일</th>
                    <td>{{ (empty($arr_adminInfo['UpdDatm']) === true) ? '' : $arr_adminInfo['UpdDatm'] }}</td>
                </tr>
            </table>
            ...
            <script type="text/javascript">
                ...
                $(document).ready(function() {
                    ...
                    $regi_form.submit(function () {
                        var _url = '{{ site_url('/predict2/base/code/storeSubject') }}';

                        ajaxSubmit($regi_form, _url, function (ret) {
                            if (ret.ret_cd) {
                                notifyAlert('success', '알림', ret.ret_msg);
                                $("#pop_modal").modal('toggle');
                                location.replace('{{ site_url('/predict2/base/code/') }}' + dtParamsToQueryString($datatable));
                            }
