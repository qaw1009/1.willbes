...
<div class="willbes-Leclist c_both">
    <div class="LeclistTable">
        <table cellspacing="0" cellpadding="0" class="listTable upper-gray upper-black bdb-gray tx-gray">
            ...
            <tbody>
            @if(empty($tab_data['list']) === true)
                <tr>
                    <td class="w-list tx-center" colspan="7">등록된 내용이 없습니다.</td>
                </tr>
            @else
                @foreach($tab_data['list'] as $row)
                <tr class="{{($row['IsBest'] == 1) ? 'strong' : ''}}">
                    <td class="w-no">@if($row['IsBest'] == '1')<img src="{{ img_url('prof/icon_HOT.gif') }}">@else{{$tab_data['paging']['rownum']}}@endif</td>
                ...
                @php $tab_data['paging']['rownum']-- @endphp
                @endforeach
            @endif
            </tbody>
        </table>
