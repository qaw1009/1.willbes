...
@section('content')
    <h5>- {{ $consult_name }} 예약일정을 등록하고 관리하는 페이지입니다.</h5>
    {!! form_errors() !!}
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#btn_list').click(function() {
                location.href='{{ site_url($default_path . '/schedule/') }}' + getQueryString();
            });

            //데이터 수정 폼
            $('#btn_modify').click(function() {
                location.href='{{ site_url($default_path . '/schedule/create') }}/' + {{$cs_idx}} + getQueryString();
            });

            //데이터 삭제
            $('#btn_delete').click(function() {
                var _url = '{{ site_url($default_path . '/schedule/delete') }}/' + {{$cs_idx}} + getQueryString();
                var data = {
                ...
                sendAjax(_url, data, function(ret) {
                    if (ret.ret_cd) {
                        ...
                        location.replace('{{ site_url($default_path . '/schedule/') }}' + getQueryString());
                    }
            ...
            $('.btn-schedule-member-read').click(function() {
                ...
                $('.btn-schedule-member-read').setLayer({
                    'url' : '{{ site_url($default_path . '/schedule/detailMemberModal/') }}' + uri_param,
                    'width' : 900
                });
            });
        });
