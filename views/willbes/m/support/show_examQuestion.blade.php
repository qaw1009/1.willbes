    ...
    <div id="Container" class="Container NG c_both">
        ...
        <div class="lineTabs lecListTabs c_both">
                    ...
                    <tr class="flie">
                        <td class="w-file NGR">
                            @foreach($data['AttachData'] as $row)
                                <a href="{{front_url($default_path.'/examQuestion/download?file_idx=').$row['FileIdx'].'&board_idx='.$board_idx }}" target="_blank">
            ...
            <div class="lecSubject mt40">
                <a href="{{front_url($default_path.'/examQuestion/index'.(empty($__cfg['CateCode']) === false ? '/cate/'.$__cfg['CateCode'] : '').'?'.$get_params)}}">목록</a>
            </div>
        </div>
