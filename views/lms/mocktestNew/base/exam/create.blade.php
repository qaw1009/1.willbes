        ...
        <div class="x_content">
            <form class="form-horizontal form-label-left" id="regi_form" name="regi_form" method="POST" enctype="multipart/form-data" onsubmit="return false;" novalidate>
                        ...
                        <div id="selected_category" class="row mt-10">
                            @foreach($cate_data as $row)
                                <div class="col-xs-5 pb-5">
                                    ...
                                    <input type="hidden" name="moLink[]" value="{{$row['MrsIdx']}}">
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="col-md-10 form-inline">
                        ...
                        <span id="selected_area" class="mt-10">
                            @if ($method == 'PUT')
                                ...
                                <a href="#none" data-area-code="{{$data['MaIdx']}}" class="selected-area-delete"><i class="fa fa-times red"></i></a>
                                <input type="hidden" name="area_code" value="{{$data['MaIdx']}}">
                            @endif
                        </span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <label class="control-label col-md-1-1" for="is_use_y">기본배점</label>
                    <div class="col-md-5 form-inline">
                        <select class="form-control mr-5" id="BaseScoring" name="BaseScoring" title="기본배점">
                            <option value="">기본배점</option>
                            <option value="5" @if($method == 'PUT' && $data['BaseScoring'] == '5') selected @endif>5점</option>
                            <option value="4" @if($method == 'PUT' && $data['BaseScoring'] == '4') selected @endif>4점</option>
                            <option value="2.5" @if($method == 'PUT' && $data['BaseScoring'] == '2.5') selected @endif>2.5점</option>
                        </select>
                        <span class="ml-20">※ 성적처리/통계 > 점수분포표 점수 기준 셋팅 설정 (기본 2.5점 기준).</span>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('.act-searchCate, .act-searchArea, .act-searchProfessor').on('click', function() {
                ...
                if( $(this).hasClass('act-searchCate') ) {
                    $('.act-searchCate').setLayer({
                        /*'url' : '{{ site_url() }}' + 'mocktestNew/base/code/moCate?reg=Y&single=Y&siteCode=' + $('[name=siteCode]').val(),*/
                        'url' : '{{ site_url() }}' + 'mocktestNew/base/code/moCate?reg=Y&siteCode=' + $('[name=siteCode]').val(),
                        'width': 1100
                    });
                } else if($(this).hasClass('act-searchArea')) {
                    $('.act-searchArea').setLayer({
                        'url' : '{{ site_url() }}' + 'mocktestNew/base/range/searchArea?siteCode=' + $('[name=siteCode]').val(),
                        'width': 900
                    });
            ...
            $('#goList').on('click', function() {
                location.replace('{{ site_url('/mocktestNew/base/exam/') }}' + getQueryString());
            });

            // 기본정보 등록,수정
            $regi_form.submit(function() {
                var _url = '{{site_url('/mocktestNew/base/exam/store')}}';
            ...
            $regi_sub_form.submit(function () {
                ...
                var _url = '{{ site_url('/mocktestNew/base/exam/storeQuestion') }}';
            ...
            $('#act-sort').on('click', function () {
                ...
                var _url = '{{ site_url("/mocktestNew/base/exam/sort") }}';
                var data = {
