<div class="acadBoxWrap">
    ...
    @if($arr_input['tab'] == 'on_before_lecture')
        <div id="on_before_lecture" class="tabContent">
    ...
    @if($arr_input['tab'] == 'off_before_lecture')
        <div id="off_before_lecture" class="tabContent">
            <form id="regi_off_form" name="regi_off_form" method="POST" onsubmit="return false;" novalidate>
                {!! csrf_field() !!}
                {!! method_field('POST') !!}
                <input type="hidden" name="learn_pattern" value="off_lecture_before"/>  {{-- 학습형태 --}}
                <input type="hidden" name="cart_type" value=""/>   {{-- 장바구니 탭 아이디 --}}
                <input type="hidden" name="is_direct_pay" value=""/>    {{-- 바로결제 여부 --}}
                ...
                <div class="willbes-Lec NG c_both">
                    ...
                    @foreach($tab_data['off_lecture_before'] as $idx => $row)
                        <div class="willbes-Lec-Table">
                            <table cellspacing="0" cellpadding="0" class="lecTable">
                                <colgroup>
            ...
            {{-- 학원 단과 footer script --}}
            @include('willbes.pc.site.off_lecture.only_footer_partial', ['learn_pattern' => 'off_lecture_before'])
        </div>
        <!-- off_before_lecture -->
    @endif
    </div>
