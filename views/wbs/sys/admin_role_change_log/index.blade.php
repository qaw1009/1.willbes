...
@section('content')
    <h5>- WBS 내부 운영자 권한 변경 이력을 확인하는 메뉴입니다.</h5>
            ...
            <div class="x_content">
                <div class="form-group">
                    ...
                    <div class="col-md-5 form-inline">
                        <select class="form-control" id="search_prev_role_idx" name="search_prev_role_idx" title="이전권한">
                            <option value="">이전권한</option>
                            <option value="0">권한없음</option>
                            @foreach($roles as $key => $val)
                                <option value="{{ $key }}">{{ $val }}</option>
                            @endforeach
                        </select>
                        <select class="form-control" id="search_role_idx" name="search_role_idx" title="변경권한">
                            <option value="">변경권한</option>
                            <option value="0">권한없음</option>
                            @foreach($roles as $key => $val)
                                <option value="{{ $key }}">{{ $val }}</option>
                            @endforeach
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $datatable = $list_table.DataTable({
                ...
                ajax: {
                    'url' : '{{ site_url('/sys/adminRoleChangeLog/listAjax') }}',
                    'type' : 'POST',
                    'data' : function(data) {
            ...
            $('.btn-excel').on('click', function(event) {
                ...
                if (confirm('정말로 엑셀다운로드 하시겠습니까?')) {
                    formCreateSubmit('{{ site_url('/sys/adminRoleChangeLog/excel') }}', $search_form.serializeArray(), 'POST');
                }
            });
        });
    </script>
