...
class Anonymous extends BaseBoard
{
    ...
    public function listAjax()
    {
        ...
        if(empty($list) === false) {
            foreach ($list as $key => $val) {
                if(empty($val['Title']) === false) $list[$key]['Title'] = htmlspecialchars($val['Title']);
    ...
    public function create($params = [])
    {
        ...
        if (empty($params[0]) === false) {
            $column = '
                ...
                LB.AreaCcd, LB.SubjectIdx, PS.SubjectName, LB.CampusCcd
            ';
            $method = 'PUT';
            $board_idx = $params[0];
            $arr_condition = ([
    ...
    public function read($params = [])
    {
        ...
        $column = '
            ...
            LB.RegNickName, MEM.MemId
                    
        ';
//        $column = '
//            LB.BoardIdx, LB.RegType, LB.SiteCode, LS.SiteName, LB.Title, LB.Content, LB.RegAdminIdx, LB.RegDatm, LB.RegNickName, MEM.MemId, LB.IsBest, LB.IsUse, LB.ExamProblemYear,
//            LB.ReadCnt, LB.SettingReadCnt, LBA.AttachFileIdx, LBA.AttachFilePath, LBA.AttachFileName, LBA.AttachRealFileName, ADMIN.wAdminName, ADMIN2.wAdminName AS UpdAdminName, LB.UpdDatm,
//            LB.AreaCcd, LB.SubjectIdx, PS.SubjectName
//        ';
        $board_idx = $params[0];
