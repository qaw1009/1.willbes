...
@section('layer_title')
    ...
    @section('layer_content')
        <div>
            ...
            <div class="form-group form-group-sm">
                <div class="col-md-6">
                    <div class="row">
                        ...
                        <div class="form-group alim-view">
                            <label class="control-label col-md-4" for="tmpl_cd">템플릿 유형<span class="required">*</span></label>
                            <div class="col-md-8 item">
                                <select class="form-control" id="tmpl_cd" name="tmpl_cd" required="required" title="">
                                    <option value="">템플릿선택</option>
                                    @foreach($arr_kakao_template as $key => $val)
                                        <option value="{{ $val['TmplCd'] }}" data-msg="{{ $val['Msg'] }}">{{ $val['TmplName'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            <div class="mt-10">
                <span class="red">
                    ※ SMS/LMS로 전송 시 카카오톡으로 우선 발송 됩니다. (윌비스 플러스 친구 등록 회원에 한함)
                    <br/>
                    ※ 메세지 내용은 공백포함 1000글자, 2000바이트 이내로 전송 가능합니다.
                </span>
            </div>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                // 알림톡 구분
                $modal_regi_form.on('ifChanged ifCreated', 'input[name="kakao_msg_type"]', function() {
                    var kakao_msg_type = $(this).val();
                    if(kakao_msg_type == 'KFT'){
                        $('.sms-view').show();
                        $('.alim-view').hide();
                    }else if(kakao_msg_type == 'KAT'){
                        $('.sms-view').hide();
                        $('.alim-view').show();
                    }
                });
                ...
                // 알림톡 템플릿 변경
                $modal_regi_form.on('change', 'select[name="tmpl_cd"]', function() {
                    $('#send_content').val( $(this).find('option:selected').data('msg') );
                    chkByteContent();
                });
                ...
                $modal_regi_form.submit(function () {
                    ...
                    var _url = '{{ site_url('/crm/sms/storeSendKakao') }}';
