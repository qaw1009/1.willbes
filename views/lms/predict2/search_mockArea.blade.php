    ...
    @section('layer_content')
        ...
        <div class="mt-10 mb-50">
            <table id="list_table_modal" class="table table-bordered table-striped table-head-row2">
                ...
                @foreach($data as $row)
                    <tr>
                        <td class="text-center"><input type="radio" class="flat btn-apply" id="_area_code_{{$row['PaIdx']}}" name="_area_code" value="{{ $row['PaIdx'] }}"></td>
                        ...
                        <td class="text-center area-title-{{$row['PaIdx']}}">{{ $row['QuestionArea'] }}</td>
