            ...
            @foreach($tab_data['on_lecture'] as $idx => $row)
                ...
                @endif
                    ...
                    <dl class="w-info tx-gray">
                        <dt>학원실강의 : {{ empty($row['StudyStartDate']) ? '' : substr($row['StudyStartDate'], 0, 4) . '년 ' . substr($row['StudyStartDate'], 5, 2) . '월' }}</dt><br>
        ...
        @foreach($tab_data['on_pack_normal'] as $row)
            <tr>
                <td class="w-data tx-left">
                    ...
                    <div class="w-tit">
                        <a href="{{ front_url('/package/show/cate/', false, true).$row['CateCode'].'/pack/'.$pack.'/prod-code/'.$row['ProdCode'] }}">{{ $row['ProdName'] }}</a>
                    </div>
                    <dl class="w-info tx-gray">
