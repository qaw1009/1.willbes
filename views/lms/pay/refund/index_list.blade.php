...
@section('content')
    <h5>- 결제관리> 환불관리> 환불처리 메뉴에서 환불완료 처리된 내역을 확인할 수 있습니다. (주문번호 기준이므로 결제완료 상품이 포함될 수 있음)</h5>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            if ($search_form.find('input[name="search_start_date"]').val().length < 1 || $search_form.find('input[name="search_end_date"]').val().length < 1) {
                setDefaultDatepicker(-2, 'months', 'search_start_date', 'search_end_date');
            }
            ...
            $datatable = $list_table.DataTable({
                ...
                ajax: {
                    'url' : '{{ site_url('/pay/refundList/listAjax') }}',
                    'type' : 'POST',
                    'data' : function(data) {
            ...
            $('.btn-excel').on('click', function(event) {
                ...
                if (confirm(confirm_msg)) {
                    formCreateSubmit('{{ site_url('/pay/refundList/excel') }}', $search_form.serializeArray(), 'POST');
                }
            });

            // 데이터 수정 폼
            $list_table.on('click', '.btn-view', function() {
                location.href = '{{ site_url('/pay/refundList/show') }}/' + $(this).data('idx') + dtParamsToQueryString($datatable);
            });
        });
    </script>
@stop
