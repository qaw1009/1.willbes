...
@section('content')
    <h5>- 사이트 개인정보처리방침을 관리하는 메뉴입니다.</h5>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $datatable = $list_table.DataTable({
                ...
                buttons: [
                    { text: '<i class="fa fa-pencil mr-5"></i> 등록', className: 'btn-sm btn-primary border-radius-reset', action: function(e, dt, node, config) {
                            location.href = '{{ site_url('/site/terms/privacy/create') }}' + dtParamsToQueryString($datatable);
                        }}
                ],
                ajax: {
                    'url' : '{{ site_url('/site/terms/privacy/listAjax') }}',
                    'type' : 'POST',
                    'data' : function(data) {
            ...
            $list_table.on('click', '.btn-modify', function() {
                location.replace('{{ site_url('/site/terms/privacy/create') }}/' + $(this).data('idx') + dtParamsToQueryString($datatable));
            });
        });
    </script>
@stop
