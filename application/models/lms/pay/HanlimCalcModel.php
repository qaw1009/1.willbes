...
class HanlimCalcModel extends BaseOrderModel
{
     ...
     * @return mixed
     */
    public function listCalcHist($search_type, $search_param1, $search_param2, $site_code = null, $is_count = false, $arr_condition = [], $limit = null, $offset = null, $order_by = [])
    {
        if ($is_count === true) {
            $column = 'count(*) AS numrows';
        } else {
            $column = 'TA.ProfIdx, TA.ProdCode
                ...
                , (select count(0) 
                    ...
                    where ProdCode = TA.ProdCode 
                        and PayStatusCcd = "' . $this->_pay_status_ccd['paid'] . '") as LecRealCnt
                , (select count(0) 
                    ...
                    where A.ProdCodeSub = TA.ProdCode
                        and B.PayStatusCcd = "' . $this->_pay_status_ccd['paid'] . '") as PackRealCnt              
            ';
        ...
        $query = /** @lang text */ '
            select ' . $column . '
            from (
                ' . $this->_getCalcHistBaseQuery($search_type, $search_param1, $search_param2, $site_code) . '
            ) as TA  
     ...
     * @return mixed
     */
    public function findCalcHist($prof_idx, $prod_code, $pch_idx = null)
    {
        $arr_condition = ['EQ' => ['PCH.PchIdx' => $pch_idx]];
        $data = $this->listCalcHist('FixedValue', $prof_idx, $prod_code, null, false, $arr_condition, 1, 0);
     ...
     * @return string
     */
    private function _getCalcHistBaseQuery($search_type, $search_param1, $search_param2, $site_code = null)
    {
        if ($search_type == 'CalcDate') {
            ...
            $query = /** @lang text */ '
                select PCH.ProfIdx, PCH.ProdCode
            ...
            // 교수식별자/상품코드 지정
            $query = /** @lang text */ '
                select ' . $this->_conn->escape($search_param1) . ' as ProfIdx, ' . $this->_conn->escape($search_param2) . ' as ProdCode
            ';
        } else {
            // 수강일/종강일 기준
            $query = /** @lang text */ '
                select PD.ProfIdx, P.ProdCode
                ...
                where P.ProdTypeCcd = "' . $this->_prod_type_ccd['off_lecture'] . '"
                    ...
                    and P.SiteCode = ' . $this->_conn->escape($site_code) . '
                union
                select SPD.ProfIdx, PRS.ProdCodeSub as ProdCode
                from ' . $this->_table['product'] . ' as P
                    inner join ' . $this->_table['product_lecture'] . ' as PL
                        on PL.ProdCode = P.ProdCode
     ...
     * @return mixed
     */
    public function listCalcOrder($prof_idx, $prod_code, $is_sum = false, $is_excel = false, $base_datm = null)
    {
        if ($is_sum === true && $is_excel === false) {
            $column = 'sum(U.DivisionPayPrice) as tPayPrice
	            , sum(U.DivisionRefundPrice) as tRefundPrice
                , sum(U.PrePrice) as tPrePrice 
                , sum(U.PreCardPrice) as tPreCardPrice
                , sum(U.PreCashPrice) as tPreCashPrice
                , sum(U.PreBankPrice) as tPreBankPrice
                , sum(U.PreVBankPrice) as tPreVBankPrice
                , sum(U.DivisionPgFeePrice) as tPgFeePrice                
                , sum(U.RemainPrice) as tRemainPrice
                , sum(U.LecRemainPrice) as tLecRemainPrice
                , sum(U.PackRemainPrice) as tPackRemainPrice
        ...
        $query = /** @lang text */ '
            select ' . $column . '
            from (
                ' . $this->_getCalcOrderBaseQuery($prof_idx, $prod_code) . '
            ) as U                             
        ';
     ...
     * @return string
     */
    private function _getCalcOrderBaseQuery($prof_idx, $prod_code)
    {
        return /** @lang text */ '
            select RD.OrderIdx, RD.OrderProdIdx, RD.ProdCode, RD.ProdCodeSub
                ...
                , if(RD.PayMethodCcd in ("' . $this->_pay_method_ccd['card'] . '", "' . $this->_pay_method_ccd['visit_card'] . '", "' . $this->_pay_method_ccd['visit_card_cash'] . '", "' . $this->_pay_method_ccd['admin_pay'] . '", ""), RD.DivisionCardPayPrice - RD.DivisionCardRefundPrice, 0) as PreCardPrice
                , if(RD.PayMethodCcd in ("' . $this->_pay_method_ccd['willbes_bank'] . '", "' . $this->_pay_method_ccd['visit_cash'] . '", "' . $this->_pay_method_ccd['visit_card_cash'] . '"), RD.DivisionCashPayPrice - RD.DivisionCashRefundPrice, 0) as PreCashPrice
                , if(RD.PayMethodCcd in ("' . $this->_pay_method_ccd['direct_bank'] . '"), RD.DivisionCardPayPrice - RD.DivisionCardRefundPrice, 0) as PreBankPrice
                , if(RD.PayMethodCcd in ("' . $this->_pay_method_ccd['vbank'] . '"), RD.DivisionCardPayPrice - RD.DivisionCardRefundPrice, 0) as PreVBankPrice	
                , (RD.DivisionPayPrice - RD.DivisionRefundPrice - RD.DivisionPgFeePrice) as RemainPrice
                , if(RD.LearnPatternCcd = "' . $this->_learn_pattern_ccd['off_lecture'] . '", RD.DivisionPayPrice - RD.DivisionRefundPrice - RD.DivisionPgFeePrice, 0) as LecRemainPrice
                , if(RD.LearnPatternCcd = "' . $this->_learn_pattern_ccd['off_pack_lecture'] . '", RD.DivisionPayPrice - RD.DivisionRefundPrice - RD.DivisionPgFeePrice, 0) as PackRemainPrice
            from (
    ...
    public function addCalcHist($input = [])
    {
        ...
        try {
            ...
            $pre_price = element('pre_price', $input, 0);
            ...
            if (empty($prof_idx) === true || empty($prod_code) === true || empty($base_datm) === true) {
            ...
            if ($sum_data['tPrePrice'] != $pre_price || $sum_data['tMemCnt'] != $mem_cnt) {
            ...
            foreach (array_get($input, 'deduct_n_price', []) as $idx => $deduct_price) {
                if (empty($deduct_price) === false) {
                    ...
                    $deduct_price = $this->_stripComma($deduct_price) * ($deduct_type == '+' ? 1 : -1);
            ...
            // 강사료정산대상금액
            $lec_target_price = $sum_data['tLecRemainPrice'] + $lec_deduct_price;
            $pack_target_price = $sum_data['tPackRemainPrice'] + $pack_deduct_price;

            // 정산기준계산금액
            $lec_calc_type = element('lec_calc_type', $input, 'R');
            $lec_calc_rate = $this->_stripComma(element('lec_calc_rate', $input, 0));
            ...
            $pack_calc_rate = $this->_stripComma(element('pack_calc_rate', $input, 0));
            ...
            // 최종 강사료지급액 체크
            if ($final_calc_price != $this->_stripComma(element('final_calc_price', $input, 0))) {
                throw new \Exception('최종 강사료지급액이 일치하지 않습니다. 다시 산출해 주십시오.', _HTTP_CONFLICT);
            }

            // 이전 강사료정산 이력 데이터 조회
            $arr_condition = ['EQ' => ['ProfIdx' => $prof_idx, 'ProdCode' => $prod_code]];
            ...
            if (empty($prev_data) === true) {
                ...
                $is_hist_update = $this->_conn->set('IsStatus', 'N')
                    ->where('PchIdx', $prev_pch_idx)->where('ProfIdx', $prof_idx)->where('ProdCode', $prod_code)->where('IsStatus', 'Y')
            ...
            $data = [
                ...
                'PayPrice' => $sum_data['tPayPrice'],
                'RefundPrice' => $sum_data['tRefundPrice'],
                'PrePrice' => $sum_data['tPrePrice'],
                'PgFeePrice' => $sum_data['tPgFeePrice'],
                'RemainPrice' => $sum_data['tRemainPrice'],
                'LecRemainPrice' => $sum_data['tLecRemainPrice'],
                'PackRemainPrice' => $sum_data['tPackRemainPrice'],
     ...
     */
    private function _stripComma($price)
    {
