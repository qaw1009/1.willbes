@if(empty($data['study_comment']) === false)
    ...
    <div class="smallTit mb30">
        <p><span>솔직한 <strong>수강후기</strong><a href="javascript:void(0);" class="btn-study-comment" data-board-idx="" data-prof-idx="" data-subject-idx=""><img src="{{ img_url('gosi_acad/icon_add_big.png') }}" alt="더보기"></a></span></p>
    </div>
    <div class="reviewBx">
            ...
            @foreach($data['study_comment'] as $row)
                <div class="lecReview">
                    ...
                    <ul>
                        ...
                        <li>{{ $row['ProdName'] }}</li>
                        <li><a href="javascript:void(0);" class="btn-study-comment" data-board-idx="{{ $row['BoardIdx'] }}" data-prof-idx="{{ $row['ProfIdx'] }}" data-subject-idx="{{ $row['SubjectIdx'] }}">{{ hpSubString($row['Title'], 0, 40, '...') }}</a></li>
                    </ul>
                </div>
            @endforeach
        </div>
