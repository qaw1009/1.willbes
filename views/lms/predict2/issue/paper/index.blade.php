            ...
            <div class="x_content">
                <div class="form-group form-inline">
                    ...
                    <div class="col-md-11">
                        ...
                        <select class="form-control mr-5" id="search_paper" name="search_paper">
                            <option value="">과목</option>
                            @foreach($arr_base['reg_paper'] as $k => $v)
                                <option value="{{$v['PpIdx']}}" class="{{$v['PredictIdx2']}}">{{$v['PapaerName']}}</option>
                            @endforeach
                        </select>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            $datatable = $list_table.DataTable({
                serverSide: true,
                buttons: [
                    { text: '<i class="fa fa-file-excel-o mr-5"></i> 엑셀다운로드 Research1', className: 'btn-sm btn-success border-radius-reset mr-15 btn-excel-research1' },
                    { text: '<i class="fa fa-file-excel-o mr-5"></i> 엑셀다운로드 Research2', className: 'btn-sm btn-success border-radius-reset mr-15 btn-excel-research2' },
                    { text: '<i class="fa fa-mobile mr-5"></i> SMS발송', className: 'btn btn-sm btn-primary btn-sms' }
                ],
                ajax: {
                    'url' : '{{ site_url('/predict2/issue/answerPaper/listAjax') }}',
                    'type' : 'POST',
                    'data' : function(data) {
