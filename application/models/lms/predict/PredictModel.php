 ...
 */
    ...
    public function tempDataUpload222($aa, $params)
    {
        $this->_conn->trans_begin();
        try {
            $data = [];
            ...
            foreach ($params as $key => $row) {
                if (empty($row['B']) === false) {
                    $pp[$row['A']]['1040'] = $row['B'];
                }
                if (empty($row['C']) === false) {
                    $pp[$row['A']]['1039'] = $row['C'];
                }
                if (empty($row['D']) === false) {
                    $pp[$row['A']]['1038'] = $row['D'];
                }
                if (empty($row['E']) === false) {
                    $pp[$row['A']]['1037'] = $row['E'];
                }
                if (empty($row['F']) === false) {
                    $pp[$row['A']]['1036'] = $row['F'];
                }
                if (empty($row['G']) === false) {
                    $pp[$row['A']]['1035'] = $row['G'];
                }
                if (empty($row['H']) === false) {
                    $pp[$row['A']]['1034'] = $row['H'];
                }
                if (empty($row['I']) === false) {
                    $pp[$row['A']]['1033'] = $row['I'];
                }
                if (empty($row['J']) === false) {
                    $pp[$row['A']]['1032'] = $row['J'];
                }
                if (empty($row['K']) === false) {
                    $pp[$row['A']]['1031'] = $row['K'];
                }
                if (empty($row['L']) === false) {
                    $pp[$row['A']]['1030'] = $row['L'];
                }
            }

            $string = "
    SELECT MemIdx,PrIdx,PredictIdx,TakeMockPart,TakeArea FROM 
    lms_predict_register
    WHERE 
    PredictIdx = '100005'
    AND MemIdx IN (
    SELECT MemIdx
    ...
    ,'913462'
            ...
            $data2 = $this->_conn->query($string)->result_array();
            $mem = [];
            foreach ($data2 as $key => $row) {
                $mem[$row['MemIdx']]['MemIdx'] = $row['MemIdx'];
                $mem[$row['MemIdx']]['PrIdx'] = $row['PrIdx'];
                $mem[$row['MemIdx']]['PredictIdx'] = $row['PredictIdx'];
                $mem[$row['MemIdx']]['TakeMockPart'] = $row['TakeMockPart'];
                $mem[$row['MemIdx']]['TakeArea'] = $row['TakeArea'];
            }

            $set = [];
            $i = 1;
            foreach ($pp as $midx => $row) {
                if (empty($mem[$midx]) === false) {
                    $set = [];
                    foreach ($row as $ppidx => $opoint) {
                        $set[$ppidx]['MemIdx'] = $mem[$midx]['MemIdx'];
                        $set[$ppidx]['PrIdx'] = $mem[$midx]['PrIdx'];
                        $set[$ppidx]['PredictIdx'] = $mem[$midx]['PredictIdx'];
                        $set[$ppidx]['TakeMockPart'] = $mem[$midx]['TakeMockPart'];
                        $set[$ppidx]['TakeArea'] = $mem[$midx]['TakeArea'];
                        $set[$ppidx]['PpIdx'] = $ppidx;
                        $set[$ppidx]['OrgPoint'] = $opoint;


                        /*$set[$midx][$ppidx]['MemIdx'] = $mem[$midx]['MemIdx'];
                        $set[$midx][$ppidx]['PrIdx'] = $mem[$midx]['PrIdx'];
                        $set[$midx][$ppidx]['PredictIdx'] = $mem[$midx]['PredictIdx'];
                        $set[$midx][$ppidx]['TakeMockPart'] = $mem[$midx]['TakeMockPart'];
                        $set[$midx][$ppidx]['TakeArea'] = $mem[$midx]['TakeArea'];
                        $set[$midx][$ppidx]['PpIdx'] = $ppidx;
                        $set[$midx][$ppidx]['OrgPoint'] = $opoint;*/
                    }
                    /*print_r($set);*/
                    if ($set) $this->_conn->insert_batch($this->_table['predictGradesOrigin'], $set);
                    $i++;
                    //inert
                }
            }
            ...
            /*$string = 'insert into' . ' lms_predict_grades_origin (MemIdx,PrIdx,PredictIdx,PpIdx,TakeMockPart,TakeArea,OrgPoint) values ';
            foreach ($set as $row) {
                foreach ($row as $ppidx => $data) {
                    $string .= "({$data['MemIdx']},";
                    $string .= "{$data['PrIdx']},";
                    $string .= "{$data['PredictIdx']},";
                    $string .= "{$data['PpIdx']},";
                    $string .= "{$data['TakeMockPart']},";
                    $string .= "{$data['TakeArea']},";
                    $string .= "{$data['OrgPoint']}),";
                }
            }
            echo $string;*/
            $this->_conn->trans_commit();
