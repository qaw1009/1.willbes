...
@section('content')
    <h5>- 강의 구성을 위한 기본 과목 정보를 관리하는 메뉴입니다.</h5>
        ...
        <div class="x_content">
            ...
            <table id="list_table" class="table table-striped table-bordered">
                ...
                <tr>
                    ...
                    <th class="searching">과목코드</th>
                    <th class="searching">과목명</th>
                ...
                @foreach($data as $row)
                    <tr>
                        ...
                        <td>{{ $row['SubjectIdx'] }}</td>
                        <td><a href="#" class="btn-modify" data-idx="{{ $row['SubjectIdx'] }}"><u>{{ $row['SubjectName'] }}</u></a></td>
                        <td>@if($row['IsUse'] == 'Y') 사용 @elseif($row['IsUse'] == 'N') <span class="red">미사용</span> @endif
                            <span class="hide">{{ $row['IsUse'] }}</span>
                        </td>
                        <td>
                            <input type="text" name="order_num" class="form-control input-sm" value="{{ $row['OrderNum'] }}" data-origin-order-num="{{ $row['OrderNum'] }}" data-idx="{{ $row['SubjectIdx'] }}" readonly="readonly" style="width: 50px;" />
                        </td>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('.btn-reorder').on('click', function() {
                ...
                sendAjax('{{ site_url('/product/base/subject/reorder') }}', data, function(ret) {
                    if (ret.ret_cd) {
            ...
            $('.btn-regist, .btn-modify').click(function() {
                ...
                $('.btn-regist, .btn-modify').setLayer({
                    'url' : '{{ site_url('/product/base/subject/create/') }}' + uri_param,
                    'width' : 900
                });
            });
        });
