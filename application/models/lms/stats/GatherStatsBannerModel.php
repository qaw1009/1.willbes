...
class GatherStatsBannerModel extends GatherBaseStatsModel
{
    ...
    public function getBannerSiteRank($arr_input = [])
    {
        ...
        $column = 'SB.BIdx, B.BannerName, B.LinkUrl, B.BannerFullPath, B.BannerImgName, SB.SiteCode, S.SiteName, click_count';

        $from =  '
                    from
                    (
                        SELECT
                            SiteCode, BIdx, SUM(ClickCount) AS click_count
                        FROM 
                            '. $this->_table['banner'] . $where .'
                        GROUP BY SiteCode, BIdx
                    ) SB 
                    JOIN  '. $this->_table['site']. ' S ON S.SiteCode = SB.SiteCode 
                    JOIN '. $this->_table['banner_info']. ' B ON SB.BIdx = B.BIdx and B.IsStatus=\'Y\' 
        ';

        $order_by_limit = $this->_conn->makeOrderBy(['click_count' => 'DESC', 'SB.BIdx' => 'DESC', 'SB.SiteCode' => 'ASC'])->getMakeOrderBy();
    ...
    public function getBannerSiteLowRank($arr_input = [])
    {
        ...
        $order_by_limit = $this->_conn->makeOrderBy(['click_count' => 'ASC', 'SB.BIdx' => 'DESC', 'SB.SiteCode' => 'ASC'])->getMakeOrderBy();
        $order_by_limit .= $this->_conn->makeLimitOffset('20')->getMakeLimitOffset();
        return $this->_conn->query('select ' . $column . $from . $order_by_limit)->result_array();
    }
