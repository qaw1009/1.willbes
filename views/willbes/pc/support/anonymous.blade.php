...
<div id="Container" class="subContainer widthAuto c_both">
        ...
        <div class="willbes-Prof-Subject pl-zero NG tx-dark-black">
            ...
            <div class="willbes-Lec-Search GM f_right">
                <div class="inputBox p_re">
                    <input type="text" id="s_keyword" name="s_keyword" class="labelSearch" value="{{ element('s_keyword', $arr_input) }}" placeholder="제목, 내용, 작성자를 입력해 주세요" maxlength="30">
        ...
        <div class="willbes-Leclist c_both">
            <div class="willbes-Lec-Selected tx-gray">
                ...
                <div class="search-Btn btnAuto90 h27 f_right">
                    <button type="button" id="btn_anonymous_create" class="mem-Btn bg-blue bd-dark-blue">
                        <span>글쓰기</span>
                    </button>
                </div>
            </div>
                    ...
                    @foreach($list as $row)
                        <tr>
                            ...
                            <td class="w-list tx-left pl20 {{($row['IsBest'] == 1) ? 'strong' : ''}}">
                                ...
                                @endif
                                @if((empty($arr_base['prof_data']['IsBoardPublic']) === false && $arr_base['prof_data']['IsBoardPublic'] == 'Y') && $row['IsBest'] == 0 && $row['IsPublic'] == 'N')
                                    <img src="{{ img_url('prof/icon_locked.gif') }}">
                                @endif
                                    {{hpSubString($row['Title'],0,40,'...')}}
                                @if(empty($row['TotalCommentCnt']) === false && $row['TotalCommentCnt'] != 0)
                                    <b>[{{$row['TotalCommentCnt']}}]</b>
                                @endif
                            ...
                            <td class="w-write">{{$row['RegNickName']}}</td>
                            <td class="w-date">{{$row['RegDatm']}}</td>
                            <td class="w-answer">{{$row['TotalReadCnt']}}</td>
                        </tr>
    ...
    <script type="text/javascript" language="JavaScript">
        $(document).ready(function() {
            ...
            $('#btn_anonymous_create').click(function () {
                if(is_login != true) {
                    alert('로그인 후 이용해 주십시오.');
                    return;
                }
                location.href = "{!! front_url($default_path.'/create/cate/'.$__cfg['CateCode'].'?'.$get_params) !!}";
            });
        });
    </script>
@stop