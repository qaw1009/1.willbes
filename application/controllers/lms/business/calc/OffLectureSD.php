...
class OffLectureSD extends \app\controllers\BaseController
{
    ...
    public function excel()
    {
        ...
        $headers = ['교수명', '상품구분', '상품명', '캠퍼스', '단과반명', '개강일', '종강일', '인원', '매출금액(C)', '환불금액(D)', '순매출(F)', '결제수수료(E)'
        ...
        $results[] = [
            ...
            'tDivisionRefundPrice' => $sum_data['tDivisionRefundPrice'], 'tDivisionRemainPrice' => $sum_data['tDivisionRemainPrice'],
            'tDivisionPgFeePrice' => $sum_data['tDivisionPgFeePrice'], 'tDivisionCalcPrice' => $sum_data['tDivisionCalcPrice'],
    ...
    public function orderListExcel()
    {
        ...
        $headers = ['주문번호', '회원명', '회원아이디', '결제루트', '결제수단', '결제금액(A)', '결제일', '환불금액(D1)', '환불완료일', '결제수수료율(E2)', '결제수수료(E1)', '결제상태'
            ...
            , '안분율(B)', '안분매출(C)', '안분환불(D)', '순매출(F)', '안분수수료(E)', '정산율(G)', '정산금액(H)'];
