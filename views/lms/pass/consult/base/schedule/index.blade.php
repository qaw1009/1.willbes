    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $datatable = $list_table.DataTable({
                ...
                buttons: [
                    ...
                    { text: '<i class="fa fa-pencil mr-10"></i> 일정등록', className: 'btn-sm btn-primary border-radius-reset', action: function(e, dt, node, config) {
                        location.href = '{{ site_url($default_path . '/schedule/create') }}' + dtParamsToQueryString($datatable);
                    }}
                ],
                ajax: {
                    'url' : '{{ site_url($default_path . '/schedule/listAjax?') }}',
                    'type' : 'POST',
                    'data' : function(data) {
            ...
            $list_table.on('click', '.btn-modify', function() {
                location.href='{{ site_url($default_path . '/schedule/create') }}/' + $(this).data('idx') + dtParamsToQueryString($datatable);
            });

            // 데이터 Read 페이지
            $list_table.on('click', '.btn-read', function() {
                location.href='{{ site_url($default_path . '/schedule/read') }}/' + $(this).data('idx') + dtParamsToQueryString($datatable);
            });
