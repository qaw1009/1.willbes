...
@section('content')
    ...
    <div class="mt-20">* 문항세트를 등록하는 페이지입니다.</div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $datatable = $list_table.DataTable({
                ...
                serverSide: true,
                ajax: {
                    'url' : '{{ site_url('/predict/survey/questionsetlist') }}',
                    'type' : 'POST',
                    'data' : function(data) {
                ...
                columns: [
                    ...
                    {'data' : null, 'class': 'text-center', 'render' : function(data, type, row, meta) {
                            return '<span class="blue underline-link act-edit"><input type="hidden" name="target" value="' + row.SqsIdx + '" />' + row.SqsTitle  + '</span>';
                        }},
                    {'data' : 'SqsComment', 'class': 'text-center'},

                    {'data' : null, 'class': 'text-center', 'render' : function(data, type, row, meta) {
                            var SqsUseYn = (row.SqsUseYn === 'Y') ? '<span>사용</span>' : '<span class="red">미사용</span>';
                            return SqsUseYn;
                        }},
                ]
            });
            ...
            $list_form.on('click', '.act-edit', function () {
                ...
                location.href = '{{ site_url('/predict/survey/addquestionsetCreate') }}' + query + '&idx=' + $(this).closest('tr').find('[name=target]').val();
            });

        });

        function addquestionCreate(){
            location.href = '{{ site_url('/predict/survey/addquestionsetCreate') }}';
        }
