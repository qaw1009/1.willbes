...
class Lecture extends \app\controllers\FrontController
{
    ...
    {
        // 과정, 카테고리+과목 맵핑 정보 조회
        $arr_base = $this->api_get_data($this->restclient->getsDataJson([
        ...
        if (empty($this->_req('subject_idx')) === false) {
            $arr_base['subject2professor'] = $this->api_get_data($this->restclient->getDataJson('product/bases/subject2professor/' . $this->_site_code, [
        ...
        // 상품 조회
        $list = $this->api_get_data($this->restclient->getDataJson('product/products/apply/on_lecture/' . $this->_site_code, [
            'cate_code' => $this->_cate_code,
