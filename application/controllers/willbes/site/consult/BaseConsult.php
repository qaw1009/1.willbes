    ...
    public function index()
    {
        ...
        if ($this->_site_code == '2011' && $this->_consult_type == 'T') {
            if (element('s_campus', $arr_input, '') == '605011') {
                $arr_base['title'] = 'ㆍGS1순환 이수진 노동법 주말반 [합격생 1:1 멘토링 프로그램]';
            } else {
    ...
    private function _depth_comment($depth = 1, $arr_input = []) {
        switch ($depth) {
            case "1" :
                ...
                if ($this->_site_code == '2011' && $this->_consult_type == 'T') {
                    if (element('s_campus', $arr_input, '') == '605011') {
                        $comment = '<p style="color: red; margin-bottom: 10px;">※ 2/15(수) 오전 11시부터 예약 가능합니다.  (아이디 로그인 후 가능)</p>
                                ㆍ참여대상 : GS1순환 이수진 노동법 주말반 수강생(실강반/실영상반/온라인첨삭반/온라인수강생 해당)<br/>
                                ㆍ2/22(수) 오전 10:00까지 예약 가능하며, 선착순으로 예약 마감됩니다.<br/>
                                ㆍ진행일정<br/>
                                <span class="ml20"></span>- 2/25(토)~3/26(일) 매주 토, 일 진행<br/>
                                ...
                                ㆍ불가피 참석이 어려우신 경우, 다른 대기자분들의 상담을 위해 학원(1544-1881 ARS1, 3)으로 연락 주시기 바랍니다.';
                    } else {
