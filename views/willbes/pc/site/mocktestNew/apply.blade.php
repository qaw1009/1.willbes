                        ...
                        <div class="willbes-LecreplyList tx-gray c_both mt-zero">
                            <span class="willbes-Lec-Search willbes-SelectBox mb20 GM f_left">
                                <select id="s_cate_code" name="s_cate_code" title="응시분야" class="seleState mr10 h30 f_left">
                                    <option value="">응시분야</option>
                                    @foreach($arr_base['category'] as $row)
                                        <option value="{{$row['CateCode']}}" @if(element('s_cate_code',$arr_input) == $row['CateCode']) selected="selected" @endif>{{$row['CateName']}}</option>
                                    @endforeach
                                </select>
                            </span>
                            <span class="willbes-Lec-Search willbes-SelectBox mb20 GM f_left">
                                <select id="s_type" name="s_type" title="진행상태" class="seleState mr10 h30 f_left">
                                    <option value="">진행상태</option>
                                    @foreach($arr_base['applyType'] as $key => $val)
