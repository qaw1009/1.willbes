...
class Menu extends \app\controllers\BaseController
{
    ...
    {
        $data = $this->menuModel->listMenu(['EQ' => ['wParentMenuIdx' => $this->_reqP('menu_idx')]], null, null, ['wOrderNum' => 'asc']);
        $this->json_result(true, '', [], array_pluck($data, 'wMenuName', 'wMenuIdx'));
    }
    ...
    public function create($params = [])
    {
        ...
        if (isset($params[1]) === true) {
            ...
            $parent_menu_idx = $data['wParentMenuIdx'];
            $group_menu_idx = $data['wGroupMenuIdx'];
        }

        if ($menu_depth > 1) {
            // GNB 메뉴 목록 조회
            $parent_menus['GNB'] = $this->menuModel->listMenu(['EQ' => ['wParentMenuIdx' => 0]], null, null, ['wOrderNum' => 'asc']);
        }
        ...
        if (isset($group_menu_idx) === false) {
            // GroupMenuIdx 조회
            $group_menu_idx = ($menu_depth == 2) ? $parent_menu_idx : $this->menuModel->findMenuByMenuIdx($parent_menu_idx)['wGroupMenuIdx'];
        }
