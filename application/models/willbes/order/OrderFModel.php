...
class OrderFModel extends BaseOrderFModel
{
    public function __construct()
    {
        ...
        // 사용 모델 로드
        $this->load->loadModels(['product/productF', 'couponF', 'pointF', 'order/cartF', 'order/orderListF', 'crm/smsF']);
    }
    ...
    public function procDepositComplete($deposit_results = [])
    {
        ...
        try {
            ...
            if (isset($deposit_results['total_pay_price']) === true && $order_row['RealPayPrice'] != $deposit_results['total_pay_price']) {
                ...
                if (empty($sms_data) === false) {
                    foreach ($sms_data as $sms_row) {
                        if (empty($sms_row['SendSmsTel']) === false && empty($sms_row['SendSmsMsg']) === false) {
                            ...
                            $this->smsFModel->addKakaoMsg($sess_mem_phone, $send_sms_msg, $sms_row['SendSmsTel'], null, 'KFT');
                        }
                    }
                }
            }
