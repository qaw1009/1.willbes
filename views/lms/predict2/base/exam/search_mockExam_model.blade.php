                ...
                <table class="table table-bordered">
                    <tr>
                        ...
                        <td style="width:35%" class="form-group form-inline">
                            ...
                            {!! html_site_select($def_site_code, '_site_code', '_site_code', '', '운영 사이트', 'required', 'disabled', false) !!}
                        </td>
                ...
                $(document).ready(function() {
                    ...
                    $datatable = $list_table.DataTable({
                        ...
                        "createdRow" : function( row, data, index ) {
                            $('input[name="PpIdx[]"]').each(function(i){
                                if ($(this).val() == data['PpIdx']) {
                        ...
                        columns: [
                            ...
                            {'data' : null, 'class': '', 'render' : function(data, type, row, meta) {
                                    return '<div class="blue underline-link act-sub-apply" id="act_sub_apply_' + row.PpIdx + '" data-row-idx="' + meta.row + '">[' + row.PpIdx + '] ' + row.PapaerName + '</div>';
                                }}
                        ],
                        "drawCallback": function(settings) {
                            var api = new $.fn.dataTable.Api(settings);
                            $.each(api.rows( {page:'current'} ).data(), function(i, item) {
                                $('input[name="PpIdx[]"]').each(function(){
                                    if ($(this).val() == item.PpIdx) {
                                        $('#act_sub_apply_' + item.PpIdx).removeClass('blue underline-link act-sub-apply');
                                    }
                                });
                            });
                        }
                    ...
                    $list_table.on('click', '.act-sub-apply', function () {
                        ...
                        target.find('tr').last().find('td:eq(5)').text('['+ row.PpIdx +'] '+ row.PapaerName);
                        target.find('tr').last().find('[name="PpIdx[]"]').val(row.PpIdx);
