...
;(function(){
	...
	function keypress(e){
		...
		if( (e.keyCode == 8 && sel[1] == 0) || (e.keyCode == 37 && sel[1] == 0) )			
			setCaret( $(this).prev(':text, input[type=number], select')[0], 100);
		...
		// if the 'right' arrow key was pressed and caret was at the end -> advance to the next field
			...
			setCaret( $(this).next(':text, input[type=number], select')[0], 0);
		...
		// automatically move to the next field once user has filled the current one completely		
			...
			setCaret( $(this).next(':text, input[type=number], select')[0], 100);
	...
	function combine(){
		...
		$(this.parentNode).find(':text, input[type=number], select').each( function(){
