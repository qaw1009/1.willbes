...
@section('content')
            ...
            <div class="x_content">
                <div class="form-group">
                    ...
                    <div class="col-md-5 form-inline">
                        ...
                        <input type="hidden" name="search_promotion_code" value="{{$arr_base['promotion_code']}}">
                        <select class="form-control" id="search_view_type" name="search_view_type">
                            <option value="">전체</option>
                            <option value="1" {{ ($arr_base['view_type'] == 1 ? 'selected' : '') }}>메인</option>
                            <option value="2" {{ ($arr_base['view_type'] == 2 ? 'selected' : '') }}>이벤트</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            var view_type = '{{$arr_base['view_type']}}';
            var promotion_code = '{{$arr_base['promotion_code']}}';
            ...
            $datatable = $list_ajax_table.DataTable({
                ...
                buttons: [
                    ...
                    { text: '<i class="fa fa-pencil mr-5"></i> 상품등록', className: 'btn-sm btn-primary border-radius-reset', action: function(e, dt, node, config) {
                            location.href = '{{ site_url('/site/etc/professorHotClip/create') }}'+'?view_type='+view_type+'&promotion_code='+promotion_code;
                        }}
                ],
                ajax: {
                    'url' : '{{ site_url("/site/etc/professorHotClip/detailAjax") }}',
                    'type' : 'POST',
                    'data' : function(data) {
            ...
            $list_ajax_table.on('click', '.btn-modify', function() {
                location.href='{{ site_url('/site/etc/professorHotClip/create') }}/' + $(this).data('idx')+'?view_type='+view_type+'&promotion_code='+promotion_code;
                return false;
            });
        });
    </script>
